...
// x∨y 變成 ¬x→y 與 ¬y→x，同一變數 x 與 ¬x 不能在同一 SCC。`,
  ...
  return clauses.every(([a, b, c]) => lit(a) || lit(b) || lit(c));
    ...
    related: ["two-sat", "clique-independent-set", "hamilton-tsp"],
  },
  {
    slug: "hamilton-tsp",
    name: "哈密頓迴路與 TSP",
...
// 度量 TSP：先 Kruskal/Prim 得 MST，再加倍捷徑。`,
  ...
  },
  {
    ...
    summary:
      "給一堆整數和一個目標 T，問有沒有某些數字加起來剛好等於 T。判定是 NP-complete，但 T 不太大時可以用 DP 一格一格填，時間 Θ(nT)。T 是「數值」不是「位數」，所以叫偽多項式。",
    idea: "想像一張表：往下多考慮一個數字，往右是「能不能湊出這個和」。每格只問兩件事——不用這個數字行不行？用了、剩下的和上一列能不能湊？{3,4,5} 湊 7 時，3+4 剛好。數字可以重複用就變成零錢問題。",
    whenToUse: [
      "能否剛好付錢、檔案能否湊滿容量",
      "說明「偽多項式」與 0/1 背包的關係",
    ],
