...
export const dpExtraLessons: Algorithm[] = [
  {
    ...
    summary:
      "在一列數字裡找最長、嚴格變大、可以跳過中間項的排法。用 DP 是 O(n²)；用 tails 陣列加二分是 O(n log n)。也可以看成「和排序後序列做 LCS」。",
    idea: "每個位置問：以這個數字當結尾，前面有誰比它小、而且那個結尾已經最長。沒人可接就長度 1。O(n log n) 的寫法是維持「每個長度目前最小的結尾」，新數字用二分插入或替換。",
    whenToUse: [
      "最長上升走勢、盒子套盒（多維要排序）",
...
def lis_nlogn(a: list[int]) -> int:
  ...
  },
  {
    ...
    tags: ["DP", "字串", "對齊"],
    summary:
      "把字串 X 改成 Y，插入、刪掉、換成別的字各算 1 步，求最少幾步。表格長得像 LCS，差在對角：字一樣加 0，不一樣加 1。",
    idea: "每一格是「X 的這個前綴」改成「Y 的那個前綴」最少幾步。字相同就沿用左上；否則在刪、插、替三種裡取最小再加 1。如果只允許插和刪，距離等於 n+m−2×LCS。",
    whenToUse: [
      "拼寫校正、DNA 對齊（可改權重）",
      "diff 的簡化模型",
    ],
  ...
  },
  {
    ...
    tags: ["區間 DP", "括號化", "O(n³)"],
    summary:
      "一連串矩陣相乘，換括號不會改答案，但乘法次數差很多。DP 枚舉「最後一次乘法切在哪」，時間 Θ(n³)。這是區間 DP 的樣板。",
    idea: "先算兩個矩陣怎麼乘最省，再三個、四個，一直加長。切在中間某個位置時，成本是「左半最少 + 右半最少 + 把兩塊乘在一起」。n 個矩陣有 n+1 個維度。",
    whenToUse: [
      "括號化、最優 BST、石子合併同一型",
      "證明「結合律問題」用區間 DP",
    ],
