...
export const dpLessons: Algorithm[] = [
  {
    ...
    tags: ["DP", "遞迴式", "填表"],
    summary:
      "n 件物品，每件只能拿或不拿，背包容量 W，求最大價值。先會講「這一格在問什麼」，再寫轉移；背程式沒有定義重要。",
    idea: "每件東西只有拿或不拿。表的一列代表「輪到這件了」，一欄代表「背包還能裝多少」。格子裡是目前能拿到的最大價值：不拿就抄上一列；拿就要扣掉重量、加上價值，再跟不拿比誰大。壓成一維陣列時，容量必須從大走到小，才不會同一件被拿兩次。",
    whenToUse: [
      "選或不選、資源有限的最優化",
      "可化約成「容量 / 預算 / 時間」的組合問題",
    ],
  ...
  },
  {
    ...
    tags: ["DP", "字串", "子序列"],
    summary:
      "兩個字串裡，找出最長、順序不變但可以跳字的共同排法。填表規則固定：兩個字一樣就從左上角加 1，不一樣就看上面和左邊誰比較長。幾乎每年都有人出。",
    idea: "子序列可以跳，子字串必須連在一起，是兩題。對到同一個字，共同長度就是「少這個字時的答案 +1」；對不上，就分別丟掉 X 或 Y 的這個字，取比較長的。",
    whenToUse: [
      "diff、版本比對、DNA 序列",
